...
```
 ...
 └─────────┘          │  liệu)    │          └─────────┘
                      └───────────┘
```
> ⚠️ Lưu ý ký hiệu trong bài: bài viết `X = A·S` với **A = mixing matrix = topic** và **S = source = document-topic importance**. Đừng nhầm vai trò — A giữ "định nghĩa topic", S giữ "tài liệu nào mạnh ở topic nào".
...
🧠 Đây là mấu chốt phân biệt PCA và ICA:
- **Không tương quan:** không có quan hệ **tuyến tính** (đường thẳng) giữa hai biến. Điều kiện **yếu**.
- **Độc lập thống kê:** biết biến này **không cho biết bất cứ điều gì** về biến kia — không quan hệ tuyến tính *lẫn phi tuyến*. Điều kiện **mạnh hơn nhiều**.
🔢 Độc lập ⇒ luôn không tương quan; **nhưng không tương quan KHÔNG ⇒ độc lập**. PCA chỉ đạt "không tương quan"; ICA đòi "độc lập" → tách được các nguồn thật sự tách bạch.
📐
```
   Không tương quan nhưng VẪN phụ thuộc:
        y
        │   • •            (điểm nằm trên vòng tròn:
        │ •     •           tương quan tuyến tính = 0,
        │•       •          nhưng biết x là đoán được |y|
        │ •     •           → KHÔNG độc lập)
        │   • •
        └──────────── x
```
...
🔢 Các trục PCA **không tương quan** nhưng đó là điều kiện yếu → chúng có thể vẫn "trộn" nhiều nguồn nghĩa vào một trục.
...
```
 3 nguồn độc lập      trộn (phòng)       ICA gỡ trộn      3 nguồn phục hồi
   🎙️giọng A  ┐                          ┌→ giọng A
   🎙️giọng B  ├──►  🔊 mix1,mix2,mix3 ──►│→ giọng B
   🎙️giọng C  ┘         (micro)          └→ giọng C
```
